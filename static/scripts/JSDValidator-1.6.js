...
function JSDValidator(options)
{
    ...
    var testNumber = function(num, schema)
    {
        ...
        if(schema.Integer && num % Math.floor(num) > 0)
            return fail('errInteger', schema, num);
        ...
        if(schema.Values)
        {
            for(var i=0; i<array.length; i++)
            {
                ...
                for(var j=0; j<schema.Values.length; j++)
                {
                    ...
                    else if(schema.Values.length===1)
                    {
                        return false;
                    }
                }
            ...
            [
                {
                    Name:"Type",
                    Type:"String",
                    Values: ["String","Number","Boolean","Date","Array","Object","Boolean","Function","MultiType"]
                },
                {
                    Name:"CanBeNull",
                    Type:"Boolean",
    ...
    var fail = function(id, schema, obj, params)
    {
        ...
        try
        {
            stringified = JSON.stringify(obj,undefined,2);
        }
        catch(e)
        {
            if(e.type=="circular_structure")
                stringified = 'circulared object'; 
        }
        ...
        switch(id)
        {
            //general messages
            case 'errWrongType':
                return returnFalse(typeof(obj) + " is not a "+schema.Type+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errNoMatchedValue':
                return returnFalse(typeof(obj) + " does not match any of the given values ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errMatchedNotValue':
                return returnFalse(typeof(obj) + " matches one of the NotValues ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errConditionNotMet':
                return returnFalse("Condition did not get met on "+typeof(obj)+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errUnfoundNonOptional':
                return returnFalse(typeof(obj) + " not found while not optional ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errCantBeNull':
                return returnFalse("Object can't be null ["+(schema.Name?schema.Name+": ":"")+""+stringified+"]");
            case 'errValidate':
                return returnFalse(typeof(obj) + " did not pass the Validate function ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
    
            //string messages
            case 'errMinLength':
                return returnFalse("String is smaller than MinLength "+schema.MinLength+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");       
            case 'errMaxLength':
                return returnFalse("String is greater than MaxLength "+schema.MaxLength+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errRegExp':
                return returnFalse("String does not validate with RegExp "+schema.RegExp.toString()+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            
            //number messages
            case 'errMax':
                return returnFalse("Number is bigger than Max "+schema.Max+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errMin':
                return returnFalse("Number is smaller than Min "+schema.Min+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errSigned':
                return returnFalse("Number is Signed ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
             case 'errInteger':
                return returnFalse("Number is not an Integer ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            ...
            //array messages
            case 'errMinSize':
                return returnFalse("Array is smaller than MinSize "+schema.MinSize+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            case 'errMaxSize':
                return returnFalse("Array is bigger than MaxSize "+schema.MaxSize+" ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            
            //object messages
            case 'errCustomAttribute':
                return returnFalse("Attribute '"+params.key+"' is a custom attribute, while the schema does not have 'CanHaveCustomAttributes' set to true ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
            
            //multitype messages
            case 'errMultiType':
                return returnFalse(typeof(obj) + " did not validate on any type of the multi types ["+(schema.Name?schema.Name+": ":"")+"'"+stringified+"']");
